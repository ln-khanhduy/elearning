...
def generate(certificate):
    ...
    draw.text((title_x, 120), title_text, fill=BORDER_COLOR, font=font_title)
    ...
    draw.text((certify_x, 340), certify_text, fill=MUTED_COLOR, font=font_info)

    # === Tên học viên ===
    name_bbox = draw.textbbox((0, 0), student_name, font=font_name)
    name_x = (WIDTH - name_bbox[2]) // 2
    draw.text((name_x, 400), student_name, fill=TEXT_COLOR, font=font_name)
