...
from apps.certificates.services.certificate_image_service import CertificateImageService
        ...
        if course_completed:
            ...
            if not cert:
                ...
                cert = CourseCertificate.objects.create(
                    ...
                    course_id=course_id,
                    enrollment=enrollment,
                    certificate_code=LearningService._generate_certificate_code(user.id, course_id),
                    pdf_url="",
                    image_url="",
                )
            ...
            if not cert.image_url:
                try:
                    image_url = CertificateImageService.upload(cert)
                    if image_url:
    ...
    def complete_course(user, course_id):
        ...
        if enrollment.status == "COMPLETED":
            ...
            if not certificate.image_url:
                try:
                    image_url = CertificateImageService.upload(certificate)
                    if image_url:
