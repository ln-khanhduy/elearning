...
class QuestionImportService:
    """
    Service xử lý import câu hỏi từ file CSV/XLSX.
    Tuân thủ kiến trúc: View → Service → Repository → Model.
    """
    ...
    def parse_csv(file):
        ...
        missing = QuestionImportService.validate_headers(reader.fieldnames)
        if missing:
    ...
    def parse_excel(file):
        ...
        # Validate headers
        missing = QuestionImportService.validate_headers(headers)
        if missing:
            wb.close()
            missing_str = ', '.join(missing)
            raise ValueError(f"Thiếu cột bắt buộc: {missing_str}")
    ...
    def preview_questions(rows):
        ...
        for i, row in enumerate(rows):
            ...
            row_errors = QuestionImportService.validate_row(row, row_num)
