...
function LessonEditorPanel({ lesson, sectionId, onClose, onSave, saving }) {
  // Khởi tạo form với dữ liệu bài học hiện tại
  ...
  // Đổ dữ liệu lesson vào form khi lesson thay đổi
  useEffect(() => {
    if (lesson) {
      setForm({
        title: lesson.title || "",
  ...
  // Xử lý chọn file tài liệu đính kèm
  ...
  // Lưu bài học (gọi API qua prop onSave)
    ...
    <div className="cw-editor-panel-inner" onKeyDown={handleKeyDown}>
      ...
      <div className="cw-editor-panel-body">
        <div className="cw-form-group">
          ...
          <input type="text" name="title" className="cw-input" value={form.title} onChange={handleChange} placeholder="VD: Giới thiệu về Python" />
        </div>

        <div className="cw-form-group">
          <label className="cw-form-label"><span className="cw-form-label-text">Mô tả</span></label>
          <textarea name="description" className="cw-textarea" value={form.description} onChange={handleChange} rows={3} placeholder="Mô tả ngắn về bài học..." />
        </div>

        <div className="cw-form-group">
          <label className="cw-form-label"><span className="cw-form-label-text">Loại nội dung</span></label>
          <select name="content_type" className="cw-select" value={form.content_type} onChange={handleChange}>
        ...
        {form.content_type === "VIDEO" && (
          <div className="cw-form-group">
            <label className="cw-form-label"><span className="cw-form-label-text">URL Video <span className="text-danger">*</span></span></label>
            <input type="text" name="video_url" className="cw-input" value={form.video_url} onChange={handleChange} placeholder="https://www.youtube.com/watch?v=..." />
      ...
      <div className="cw-editor-panel-footer">
        ...
        <button className="cw-btn cw-btn-primary" onClick={handleSave} disabled={saving || !form.title.trim()}>
